...
public class ProdutoDAO {
    ...
    public void cadastrar(Produto prod) throws ClassNotFoundException, SQLException {
        ...
        PreparedStatement comando = con.prepareStatement("insert into produtos (descricao, preco, quantidade) values (?,?,?)");
    ...
    public void atualizar(Produto prod) throws ClassNotFoundException, SQLException {
        Connection con = getConexao();
        PreparedStatement comando = con.prepareStatement("update produtos set descricao = ?, preco = ?, quantidade = ? where id = ?");
        comando.setString(1, prod.getDescricao());
        comando.setDouble(2, prod.getPreco());
         comando.setInt(3, prod.getQuantidade());
